...
from timstools import DPRINT,SCREAM, InMemoryReader, InMemoryWriter, ignored, PDICT, toggle
import functools
import logging
...
class MixInText:
	...
	def text_from_tag(self, tag):	#this was from internet but forgot about it FUCK
	...
	def button_state_change(but,state):	#in cases wher i want to change values but based on a variable not simple toggle
		'''
		Pass in the str 'toggle' to change from the current state
		or 1 to turn on and 0 to turn off
		'''
		if state == 'toggle':
	...
	def indent(self,elem, level=0):
		"""
		xml method change to xml mixin class TBD
	...
	def default_tag(self, event):	#controlls binding tags on insert,
		...
		if event.char is not '' and len(repr(event.char)) >= 3:	#no blank chars from special charachters, the and statement with > blocks keys such as ctrl + (something)
			...
			time.sleep(0.1)
#~ def tims_text_line_get(text):
	#~ for char in text.get('1.0', '2.0'+'-1c'):
...
class XmlManager(MixInText):		# Xml handling class for loading and saving, changing tags into xml
		...
		def __init__(self, text):
			...
			#~ self._load_style_tags()		#Load defaults and anyother tags present. TBD, load file controller seems to be propogating the load ? i would like to check that and see if it is a good idea or not to load when the page is first created
			self.custom_default_style = 0   # TBD implement, see help above
		def load_xml(self,data):	#data here is a str of xml #TBD allow other methods
			#~ Setups up Xml to work with change styles
			#~ all data will be added to text widget (or line by line?)
			#~ and then all styles tags will be applied
			print('starting load')
			print(data)
			holder_str_data = []
			xmlt = xml.dom.minidom.parseString(data)
			pretty_xml_as_string = xmlt.toprettyxml()
			with open('xml_on_load.txt','w') as f:
				f.write(pretty_xml_as_string)
			#~ xml_reader = InMemoryReader(data)
			#~ self.tree = ET.parse(data)					#load tags into styles	!!!
			#~ DPRINT(data,'Loaded File Data')
			with ignored(ET.ParseError):					#if parse error i.e no xml tags, just pass TBD REMOVE THIS JUST FOR TESTING
				tree = ET.fromstring(data)					#load tags into styles
				#~ root = self.tree.getroot()		#unavalibel when pasrse from str, .. bug in element tree
				auto_styles = tree.find('automatic-styles')	
				body = tree.find('body')
			for style in auto_styles:			#recreating the style options
				key = style.get('name')
				values = []
				text_properties = style.find('text-properties')
				weight			= text_properties.get('weight')	#TBD ADD MORE FONT AND TEXT COLOR OPTIONS HERE
				...
				filtered = []	#filter the none values out of the tupled and normal items
				for item in values:
					try:
			...
			self._load_style_tags()		#loading the styles into text tags 
			...
			with ignored(UnboundLocalError):	#this with handles case of no body tag i.e first creation
				...
				#apply tags to text content
				#~ holder_str_data.append(tag.text)
			
			print('FINISHED LOADING')
		
		def _load_style_tags(self):	#loading the styles into text tags 
			...
			for name, style in self.styles.items():
				...
				DPRINT(self.text.tag_names(),'TAG NAMES LOADED')
		def save_with_xml(self): # a controller type function
			...
			xmlt = xml.dom.minidom.parseString(xml_data)
			pretty_xml_as_string = xmlt.toprettyxml()
			with open('xml_on_save.txt','w') as f:
				f.write(pretty_xml_as_string)
			return xml_data						#user can save in database etc
		
		def xml_setup(self):
			root = ET.Element("document-content")					#create standard xml template
			auto_styles = ET.SubElement(root, "automatic-styles")
			body = ET.SubElement(root, "body")
			tree = ET.ElementTree(root)						# wrap it in an ElementTree instance
			self.tree = tree
			#~ for key, value in self.styles.items():					#load defaults from self.styles
				#~ new_font = tkinter.font.Font(self.text, self.text.cget("font"))	#???
				#~ for item in value:									# configure font attributes 
					#~ new_font.configure(**self.option_list[item])	#attribute list contains the option name for the setting
			#~ self.text.tag_configure(key, font=new_font)				#add tag to font
		def convert_text_to_xml(self,new_data):
			#~ Internal Method
			DPRINT(new_data,'NEW DATA')
			...
			"""	
				...
				for i in range(0, len(ranges), 2):
					...
					stop = ranges[i+1]
					#~ print(start,stop)
					return start, self.text.get(start, stop)	
			"""
			holder = {}
			body = self.tree.find('body')					#navigate to body				
			#~ DPRINT(self.styles.keys(),'ALL STYLE KEYS / TAGS')
			...
			for i, pos in enumerate(sorted(holder.keys())):	#sorting text by positions and creating xml tags
				...
				if tag_type[0] == 'T':	#change to a span tag
					tag_type = 'span'
				else:
					#~ print('tag type P')
		...
		def save_style_info(self):	# saves the style info in automatic styles xml tag info
			...
			mw = InMemoryWriter()	#memory writer, mm.data contains our tree xmlas a str to be saved
			self.tree.write(mw)
		...
		def change_style_selected(self,requested_change): # what to do on selected text
			#Removal of a tag only happens if every characheter of the selecion has that tag
				#for index in range(0,self.text.get('sel.first','sel.last'))	#length of the selected text
		...
		def change_style_non_select(self,requested_change): #what to do when no text is selected
			...
			#~ SCREAM(self.text.tag_names(cursor),'tag names')
		...
		def check_styles(self,requested_change,current,remove=False):
			...
			#check if a xml tag is already present for the required + new settings
		...
		def make_name(self,style):	#
			'''makes a new name that will be used in the xml tag,
			as well as tkinter text tag
			...
			for num in versions:	#generate a unique number
				if last not in versions:	#num is  uniquie (will return numbers if the list has skips so p1 p2 p5,
					...
					last += 1	#TBD pep 8 whitespaces again
...
if __name__ == '__main__': 
	import gui_maker
	def on_bold(event=None):
		tag_manager.change_style('bold')
	def on_underline(event=None):
		tag_manager.change_style('underline')
	def on_italic(event=None):
	...
	#~ tag_manager.start.styles = {'default':[]}		#defaults here
	#~ tag_manager = tag_manager(text)
	
	#on save
	#tag_manager.save_with_xml()
	
	#on load
	#~ data = tag_manager.load_xml(MY_STORED_XML_DATA)	#TBD CHANGE NAME TO LOAD NORMAL
	...
	def test():
		...
		print(text.tag_ranges('default'))
				
	
			#~ text.search(backwards = True)
			#row, col = int(row), int(col)
			#~ if col == '0' and row is not '1':	#if it is the first char of a new line
				#~ first = (text.index)
	data = text.get('1.0', tk.END+'-1c') 	#TBD DELTEE JUST FOR TEST	
			
	text.bind('<Key>',lambda event: tag_manager.default_tag(event)) # required
	text.bind('<Control_L><s>',lambda e:tag_manager.save_with_xml())
	text.bind('<Control_L><o>',lambda e:tag_manager.load_xml(data))
	text.bind('<Control_L><b>', lambda event:on_bold(event))
	text.bind('<Control_L><i>', lambda event:on_italic(event))
	text.bind('<Control_L><u>', lambda event:on_underline(event))
	text.bind('<Control_L><t>', test)
	
	but = tk.Button(root, text='test', command = lambda: test()).pack(side ='right')
	bold = tk.Button(root, text='Bold', command = lambda: tag_manager.change_style('bold'))
	bold.pack(side='left')
	italic = tk.Button(root, text='Italic', command = lambda: tag_manager.change_style('italic'))
	italic.pack(side='left')
	underline = tk.Button(root, text='Underline', command = lambda: tag_manager.change_style('underline'))
	underline.pack(side='left')
	but = tk.Button(root, text='Overstrike', command = lambda: tag_manager.change_style('overstrike')).pack(side='left')
	
	family_menu = gui_maker.MakerOptionMenu	#Subclassing my gui builder and configuring
	family_menu.options = ['Arial','Times New Roman','Trebuchet Ms','Comis Sans Ms','Verdana','Georgia']
	family_menu.initialValue = 'Font'
	family_menu.run_command = lambda self,value:tag_manager.change_style(value)
	family_menu(root)							#initalize the menu
	
	size_menu = gui_maker.MakerOptionMenu	#Subclassing my gui builder and configuring
	size_menu.options = [6,8,10,12,14,16,18]
	size_menu.initialValue = 'Size'
	size_menu(root)							#initalize the menu
	
	items = {'bold':bold,
				'italic':italic,
				'underline':underline}
	tag_manager.button_references = items #button references
